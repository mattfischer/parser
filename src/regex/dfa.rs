...
impl DFA {
    pub fn new(nfa: &NFA, encoding: &Encoding) -> DFA {
        ...
        let mut start_state = find_or_add_state(&mut state_sets, nfa, &nfa_states);
        ...
        (states, start_state, accept_states) = minimize(&states, start_state, &accept_states);
...
fn find_or_add_state(state_sets: &mut Vec<StateSet>, nfa: &NFA, nfa_states: &HashSet<usize>) -> usize {
    ...
    for transition in &transitions {
        let s = find_or_add_state(state_sets, nfa, transition.1);
