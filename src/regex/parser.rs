...
impl Parser {
    pub fn parse(regex: &str) -> Result<Node, ParseError> {
        let mut pos = 0;
        let node = parse_sequence(regex, &mut pos)?;
        if pos == regex.len() {
            return Ok(node);
        } else {
...
fn parse_sequence(regex: &str, pos: &mut usize) -> Result<Node, ParseError> {
    ...
    while *pos < regex.len() {
        match regex.chars().nth(*pos).unwrap() {
            '|' | ')' => break,
            _ => nodes.push(parse_suffix(regex, pos)?)
        }
    }
...
fn parse_suffix(regex: &str, pos: &mut usize) -> Result<Node, ParseError> {
    let mut node = parse_one_of(regex, pos)?;
    while *pos < regex.len() {
        let c = regex.chars().nth(*pos).unwrap();
        match c {
...
fn parse_one_of(regex: &str, pos: &mut usize) -> Result<Node, ParseError> {
    if regex.chars().nth(*pos).unwrap() == '(' {
        ...
        *pos += 1;
        loop {
            nodes.push(parse_sequence(regex, pos)?);
...
fn parse_symbol(regex: &str, pos: &mut usize) -> Result<Node, ParseError> {
    ...
    match c {
        ')' | '|' | '*' | '+' | '?' => return Err(ParseError::new(&format!("Unexpected character {c}"), *pos)),
        '[' => return parse_character_class(regex, pos),
        '\\' => return parse_escape(regex, pos),
        _ => {
